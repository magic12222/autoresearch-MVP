...
首个示范任务已由 P0 固定 CPU 样例完成。用户已确认小型 2D Diffusion 为首个论文复现基线；P1-2 已核实官方模板、许可、随仓数据、输入输出和指标，并以 Git submodule 固定 AI-Scientist 与 NPEET commit。默认工作量为四个数据集各 10,000 训练步，但真实墙钟预算仍须在 `labtmx56` 首次运行后测得。验收时保留输入、代码版本、命令、环境、退出状态、原始指标、图、时间和错误，不以 LLM 自述替代执行证据。
