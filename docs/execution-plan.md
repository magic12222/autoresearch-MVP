...
| P1-2 | 固定官方 2D Diffusion baseline，远端原样复现，成功后设计最薄输出 adapter | 进行中；远端环境已验证，四张 GPU 当前均被占用 | 见 `reproduction-ai-scientist-2d-diffusion.md`；训练尚未运行 |
| P1 后续 | LLM 生成想法、真实文献核对、受限改代码与排错 | 待做 | 见 `../PROJECT_PLAN.md` |
| P2–P6 | 实验树、论文、Web、恢复与完整平台 | 规划中 | 见 `../PROJECT_PLAN.md` |

用户已确认 P1-2 复现官方 2D Diffusion baseline。代码、许可、数据、输入输出、指标和远端环境已核实；下一步等待一张 GPU 真正空闲后原样运行。输出 adapter 必须等 baseline 跑通后再实现。
